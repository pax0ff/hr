...
class Angajat
{
    ...
    //select departamente.nume from departamente join angajat on departamente.id = angajat.departament;
    //select * from angajat join departamente on departamente.id = angajat.departament;
    public function getDataWithDepartmentNames() {
        $data = $this->_db->join('angajat','departamente',['id','nume','prenume','varsta','oras','functie','join_date','group_id'],'nume','depa_num',['departamente.id','=','angajat.departament']);
    ...
    public function getDepartamentNames() {
        $data = $this->_db->join('departamente',['departamente.nume'],'angajat',['departamente.id','=','angajat.departament']);
        if($data->count()) {
            $this->_data = $data;
        }
        return  $this->data()->results();
